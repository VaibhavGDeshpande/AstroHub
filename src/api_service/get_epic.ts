...
export async function getEpicImageUrl(date?: string, imageIndex: number = 0): Promise<string> {
    try {
        ...
        return `https://epic.gsfc.nasa.gov/archive/natural/${formattedDate}/jpg/${image}.jpg`;
    } catch (error) {
    ...
    try {
        ...
        const imageUrls = await Promise.all(
            epicData.map(async (_, index) => {
                return await getEpicImageUrl(date, index);
            })
        );
...
export default getEpicImageUrl;
